...
class SuperPixDGL(torch.utils.data.Dataset):
    def __init__(self,
        ...
        if dataset == 'MNIST':
            ...
            with open(os.path.join(data_dir, 'CP_%s.pkl' % split), 'rb') as f:
                tmp = pickle.load(f)
...
class SuperPixDatasetDGL(torch.utils.data.Dataset):
    def __init__(self, name, num_val=2):#num_val=5000):
        """
        ...
        temp_train = list(zip(all_train_label, all_train_data))
        for i in range(50):
...
class SuperPixDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, name):
        ...
        self.name = name
        with open('C:/Users/rezav/PycharmProjects/benchmarking-gnns/data/superpixels/'+name+'.pkl',"rb") as f:
            f = pickle.load(f)
            self.train = f[0]
            self.val = f[1]
            self.test = f[2]
